...
- **Conversation-rewind does NOT get blocked by `/clear` the way it is by
  compaction.** Confirmed by direct interactive testing: after an accidental
  `/clear`, `Esc Esc` → `/rewind` still offered a restore point for the moment
  right before the clear, and it was selectable. So `/clear`'s "wipes
  conversation" behavior only resets what's *currently shown*/active — it does
  not truncate the underlying transcript enough to invalidate it as a
  conversation-rewind target. (Contrast with compaction above, which *does*
  age messages out of valid rewind targets.) This is the one finding in this
  doc confirmed by live human interactive use rather than a string/CLI-probe
  anchor. **Unconfirmed:** whether the *code/file* rewind arm also survives a
  `/clear` — the test session had no file edits in the pre-clear window, so
  there was nothing file-side to observe. Treat that half as an open question.
...
- **`/clear`** — "wipes conversation but keeps files." Forward-only reset of the
  ...
  to a specific point and optionally restore files. "Wipes" turns out to be
  about the visible/active context, not the underlying transcript — the
  conversation arm of rewind can still reach messages from before a `/clear`
  (see above; whether the file arm can too is unconfirmed).
