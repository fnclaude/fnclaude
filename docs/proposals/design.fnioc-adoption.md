...
5. **Published extras cannot lower** — triple-verified against the current npm alphas; all four consumed extras lack the sugar bodies and Go host, not just di.extras (stance 3 J6 verified `config.extras files:["ttsc.mjs"]` — confirmed this session). Post-debate correction from the maintainer: those alphas are placeholder dummies published only to bootstrap npm configuration — the real libraries are not yet published at all. The finding therefore describes the placeholders, and PR-0 is the *first real publish* passing the probe, not the repair of a broken one.
6. **Registry-fork hazard in external runtime deps** — stance 2's A34 empirical demonstration stands unrebutted by the other stances; bundling adopted (Graft 2). (The demo ran against the placeholder alphas' caret ranges — re-verify the real publish's declared ranges at PR-0 time; the mechanism, and the bundling decision, don't depend on it.)
...
|---|---|---|---|---|
| 0 | std repo: first real extras publish (all four packages ship `src/inline.ts` + reachable Go host; current npm alphas are placeholders); optional: `serviceProviderOptions` settings knob issue (non-load-bearing) | upstream | hermetic four-extras probe green | — |
...
1. **std upstream work (PR-0).** The first real, probe-passing publish of the four extras is a hard prerequisite for every tokenless PR; std is a separate repo, so this is a scheduling call — when does that publish slot in? *Default:* do it before fnclaude wave B; fnclaude wave A proceeds meanwhile. The optional `serviceProviderOptions` settings knob is a nicety issue, not load-bearing.
