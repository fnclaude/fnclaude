...
✅ **§7.4** **Self-MCP `--mcp-config` injection** — `packages/cli/src/mcp/inject-config.ts` exports a pure `injectMcpConfig({claudeArgs, bunExec, fncBin, noop, interactive})` that returns `[...claudeArgs, '--mcp-config', JSON.stringify({mcpServers:{fnclaude:{command:bunExec, args:[fncBin,'mcp',(noop ? '--noop' : null)]}}})]` (filtered). Wired into main.ts between env composition and DUMP_PLAN; `bunExec=process.execPath` and `fncBin=realpathSync(process.argv[1])`. Skipped on win32 (no listener), skipped for `-p`/`--print` (interactive gate per design.md §29). Design: [`design.md` §29], [`design.mcp.md` §2.1].
...
✅ 🌿 **§8.2** **`fnc_switch_project`** — `packages/cli/src/mcp/handlers/switch.ts` ships `createSwitchHandler` with injectable `writeSummary` / clipboard / live-permission-mode reader / base-dir seams; production wires it into the parent dispatcher's `switch` slot via spread over `stubParentHandlers`. Required-arg validation (`destination`/`name`/`summary` → error if missing), summary written to `<base>/fnclaude-handoff-content-<16hex>.md` (mode 0600) via the new `packages/cli/src/handoff/summary-file.ts`, transfer-denylist `preserveArgs` + `applyOverrides`, live-permission-mode auto-capture when no override AND no preserved flag AND `session_id` present, relaunch argv `[magic] dest [rest] --name <name> @<summary>` stashed + trigger fired. `permission_mode: 'never'` branches to paste-flow: render the same command, copy it (`handleCopyToClipboard` reused), return `{action: 'paste_flow', message, command, clipboard_ok}`; the 'never' token is treated as a control signal, not forwarded into the rendered command. Live-mode reader defaults to null until the JSONL reader lands (§8.1/§8.2 follow-on). Design: [`design.md` §12–13], [`design.mcp.md` §4.2].

✅ 🌿 **§8.3** **`fnc_spawn_session`** — `createSpawnHandler` in `packages/cli/src/mcp/handlers/spawn.ts` validates `destination`/`name`/`summary` (no `session_id`), writes summary via the shared `packages/cli/src/handoff/summary-file.ts` (also consumed by §8.2), builds extra args from `applyOverrides([], req)` (no preservation), and dispatches through `chooseAndSpawn` in `packages/cli/src/handoff/spawn-launcher.ts`. Launcher decision: `config.autoSpawnCommand` → `$TMUX` built-in template → paste-flow fallback with `auto.spawnCommand` config hint. `cleanEnvForSpawn` in `packages/cli/src/handoff/clean-env.ts` strips `FNC_SOCKET`/`FNCLAUDE_HANDOFF`/`CLAUDE_CODE_SESSION_ID` from the launcher env. Wired into main.ts alongside §8.4's `handleCopyToClipboard` (which doubles as the paste-flow clipboard handler). The handler NEVER stashes argv or fires the trigger — the spawning session keeps running. Design: [`design.md` §20], [`design.mcp.md` §4.3].
